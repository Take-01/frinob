...
public class InputChecker {
	 ...
	 * @return メッセージのリスト
	 */
	public List<String> getMessageList(String input, String propertyName, int minLength, int maxLength, int ... number) {
	...
	/**
	 * 入力値をチェックし、不正ならメッセージを取得する。
	 * @param input 入力値
	 * @param propertyName 項目名
	 * @param number 1:半角英字 2:半角数字 3:ひらがな 4:カタカナ 5:漢字 6:全角記号 7:全角数字 8:半角記号
	 * @return メッセージS
	 */
	public String getMessage(String input, String propertyName, int ... number) {
	...
	public List<String> checkEmailAddress(String input) {
		...
		if(input == null || !input.matches(regularExpression)) {
			...
			messageList.add("メールアドレスは【半角英数字記号】を使い、【xxx@xxx.xx】の形式で入力してください。");
		}
		return messageList;
	}
}
